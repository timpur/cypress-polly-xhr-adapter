...
var notImplementedError = new MockError_1.MockError("This feature hasn't been implmented yet. Please submit an Issue or Pull Request on Github."); // implemented according to https://xhr.spec.whatwg.org/
...
function (_super) {
  ...
  function MockXMLHttpRequest() {
    ...
    _this.DONE = ReadyState.DONE; //some libraries (like Mixpanel) use the presence of this field to check if XHR is properly supported
    ...
    _this.responseType = '';
    _this.responseURL = '';
    _this._timeout = 0; // @ts-ignore: https://github.com/jameslnewell/xhr-mock/issues/45

    _this.upload = new MockXMLHttpRequestUpload_1.default();
    _this.readyState = MockXMLHttpRequest.UNSENT; // flags
  ...
  MockXMLHttpRequest.prototype.getAllResponseHeaders = function () {
    ...
    var result = Object.keys(headers).map(function (name) {
  ...
  MockXMLHttpRequest.prototype.open = function (method, url, async, username, password) {
    ...
    method = method.toUpperCase(); // let parsedURL be the result of parsing url with settingsObject’s API base URL and settingsObject’s API URL character encoding
    // if parsedURL is failure, then throw a "SyntaxError" DOMException

    var fullURL = MockURL_1.parseURL(url); // if the async argument is omitted, set async to true, and set username and password to null.
    ...
    fullURL.password = username && password || ''; // if async is false, current global object is a Window object, and the timeout attribute value
    ...
    this.req.method(method).headers({}).url(MockURL_1.formatURL(fullURL));
    this.applyNetworkError(); // if the state is not opened, run these substeps:

    if (this.readyState !== this.OPENED) {
      // set state to opened
      this.readyState = MockXMLHttpRequest.OPENED; // fire an event named readystatechange
  ...
  MockXMLHttpRequest.prototype.sendSync = function () {
    ...
    try {
      res = handle_1.sync(MockXMLHttpRequest.handlers, this.req, this.res); // if the timeout attribute value is not zero, then set the timed out flag and terminate fetching if it has not returned within the amount of milliseconds from the timeout.
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            ...
            this.dispatchEvent(new MockProgressEvent_1.default('loadstart', __assign({}, progress, {
              loaded: 0
            ...
            if (!this.isUploadComplete) {
              ...
              this.upload.dispatchEvent(new MockProgressEvent_1.default('loadstart', __assign({}, progress_1, {
                loaded: 0
              })));
          ...
          case 1:
            ...
            return [4
            /*yield*/
            , handle_1.async(MockXMLHttpRequest.handlers, this.req, this.res)];

          case 2:
            res = _a.sent(); //we've received a response before the timeout so we don't want to timeout
            ...
            if (this.isAborted || this.isTimedOut) {
              return [2
              /*return*/
              ]; // these cases will already have been handled
            }
          ...
          case 3:
            error_1 = _a.sent(); //we've received an error before the timeout so we don't want to timeout
  ...
  MockXMLHttpRequest.prototype.applyNetworkError = function () {
    ...
    this.res.status(0).reason('').headers({}).body(null);
  }; // @see https://xhr.spec.whatwg.org/#request-error-steps


  MockXMLHttpRequest.prototype.reportError = function (event) {
    // set state to done
    this.readyState = this.DONE; // unset the send() flag

    this.isSending = false; // set response to network error

    this.applyNetworkError(); // if the synchronous flag is set, throw an exception exception
    ...
    this.dispatchEvent(new MockEvent_1.default('readystatechange')); // if the upload complete flag is unset, follow these substeps:

    if (!this.isUploadComplete) {
      // set the upload complete flag
      this.isUploadComplete = true; // if upload listener flag is unset, then terminate these substeps
      // NOTE: not sure why this is necessary - if there's no listeners  listening, then the
      // following events have no impact

      var uploadProgress = calculateProgress(this.req); // fire a progress event named event on the XMLHttpRequestUpload object with 0 and 0

      this.upload.dispatchEvent(new MockProgressEvent_1.default(event, uploadProgress)); // fire a progress event named loadend on the XMLHttpRequestUpload object with 0 and 0

      this.upload.dispatchEvent(new MockProgressEvent_1.default('loadend', uploadProgress));
    }

    var downloadProgress = calculateProgress(this.res); // fire a progress event named event with 0 and 0

    this.dispatchEvent(new MockProgressEvent_1.default(event, downloadProgress)); // fire a progress event named loadend with 0 and 0
  ...
  MockXMLHttpRequest.prototype.sendRequest = function (req) {
    ...
    this.isUploadComplete = true; // if upload listener flag is unset, then terminate these subsubsteps.
    ...
    var progress = calculateProgress(this.req); // fire a progress event named progress on the XMLHttpRequestUpload object with transmitted and length

    this.upload.dispatchEvent(new MockProgressEvent_1.default('progress', progress)); // fire a progress event named load on the XMLHttpRequestUpload object with transmitted and length

    this.upload.dispatchEvent(new MockProgressEvent_1.default('load', progress)); // fire a progress event named loadend on the XMLHttpRequestUpload object with transmitted and length
  ...
  MockXMLHttpRequest.prototype.receiveResponse = function (res) {
    ...
    this.readyState = this.HEADERS_RECEIVED; // fire an event named readystatechange

    this.dispatchEvent(new MockEvent_1.default('readystatechange')); // if state is not headers received, then return
    ...
    this.readyState = this.LOADING; // fire an event named readystatechange

    this.dispatchEvent(new MockEvent_1.default('readystatechange')); // fire a progress event named progress with response’s body’s transmitted bytes and response’s
    // body’s total bytes
  ...
  MockXMLHttpRequest.prototype.handleResponseBody = function (res) {
    this.res = res; // let transmitted be response’s body’s transmitted bytes
    // let length be response’s body’s total bytes.

    var progress = calculateProgress(res); // if the synchronous flag is unset, update response’s body using response
    ...
    this.isSending = false; // fire an event named readystatechange

    this.dispatchEvent(new MockEvent_1.default('readystatechange')); // fire a progress event named load with transmitted and length

    this.dispatchEvent(new MockProgressEvent_1.default('load', progress)); // fire a progress event named loadend with transmitted and length
  ...
  MockXMLHttpRequest.prototype.send = function (body) {
    ...
    this.isUploadComplete = false; // unset the timed out flag

    this.isTimedOut = false; // if req’s body is null, set the upload complete flag
  ...
  MockXMLHttpRequest.prototype.abort = function () {
    ...
    clearTimeout(this._timeoutTimer); // terminate the ongoing fetch with the aborted flag set

    this.isAborted = true; // if state is either opened with the send() flag set, headers received, or loading,
    // run the request error steps for event

    if (this.readyState === this.OPENED || this.readyState === this.HEADERS_RECEIVED || this.readyState === this.LOADING) {
      this.reportError('abort');
